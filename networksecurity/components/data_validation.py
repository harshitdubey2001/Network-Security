...
class DataValidation:
    ...
    def _is_numerical(self, dataframe: pd.DataFrame) -> bool:
     try:
    ...
    def initiate_data_validation(self) -> DataIngestionArtifacts:
        try:
            ...
            status=self._is_numerical(dataframe=train_dataframe)
            if not status:
                error_message=f"train dataframe does not contain anumeric columns.\n"

            status=self._is_numerical(dataframe=test_dataframe)
            if not status:
                error_message=f"test dataframe does not contain anumeric columns.\n"
